...
# Now make contour plots
    plot_contour_pdf(p_out['m2'], p_out['mtot'], np.transpose(p_out['norm_like']),
                     ...
                     ylabel='Total system mass ($M_\\odot$)')
    plt.savefig(outfile_base+'_m2mtot_contours.png')
...
# Now plot 1D pdfs for m2 and mtot
    ...
    plt.savefig(outfile_base+'_m2_m2mtot_pdf.png')
    print 'M2 = ', m2_med
    ...
    plot_pdf(p_out['mtot'], mtot_pdf, 
             ...
             prob_linestyle=['dashed','dashdot','dotted', 
                             'dashed','dashdot','dotted'])
    plt.savefig(outfile_base+'_mtot_m2mtot_pdf.png')
    print 'MTOT = ', mtot_med
...
# And now make weighted histogram of m1 values
    m1_pdf, bin_edges = np.histogram(p_out['m1'], 32, 
                                     range=(np.amin(p_out['m1']), np.amax(p_out['m1'])),
                                     density=True, weights=p_out['norm_like'])
    ...
    plot_pdf(m1_x, m1_pdf, 
             ...
             prob_linestyle=['dashed','dashdot','dotted', 
                             'dashed','dashdot','dotted'])
    plt.savefig(outfile_base+'_m1_pdf.png')
    print 'M1 = ', m1_med
