...
#      much identical.
#  (c) We make no assumption (for now) about more than one pole of emission, but 
#      we allow more than one data set to be used for a simultaneous fit, e.g. 
#      for multiple pulse fractional height measurements.  This
#      means that all calculations will use the same value of alpha.
#  (d) We will add the ability to assume that pole 2 is opposite to pole 1, thus 
#      alpha(2) = 180 - alpha(1), but for now not sure how to combine this with
#      multiple fractional height width measurements being fit together.
####
...
#    sys.stdout.write('{0:<10d}[{1:>3d}%]  {2:8.4f}  {3:8.4f}  {4:10.4f}  {5:8.4f} +/- {6:8.4f}  {7:8.4f} +/- {8:8.4f}  {9:14.2g}  {10:14.2g}'.format( \
    sys.stdout.write('{0:<10d}[{1:>3d}%]  {2:8.4f}  {3:8.4f}  {4:17.4f}   {5:8.2g}  {6:10.2g}'.format( \
...
def profile_shape_fit(params_in, data_in):
...
# Calculate the following based on input parameters:
    n_alpha = np.sum(alpha_sampling)
    n_delta = np.sum(delta_sampling)
    n_T1 = np.sum(T1_sampling)
    alpha_min = np.amin(alpha_ranges)
    alpha_max = np.amax(alpha_ranges)
    delta_min = np.amin(delta_ranges)
    delta_max = np.amax(delta_ranges)
    T1_min = np.amin(T1_ranges)
    T1_max = np.amax(T1_ranges)

    # Set number of rho values to fit
    n_rho = len(rho_guess)

# Will assume that MJD, y and y_err are N-D numpy arrays, one for each 
# width height.  
    mjd = data_in['mjd']
    y = data_in['y']
    y_err = data_in['y_err']
...
#####
    ...
    # Set up parameters that we will not be fitting, i.e. the grid parameters  
    # that will be held constant for each fit at each point in the grid.
    # Initialize with arbitrary values, and min/max boundary values.
    params=Parameters()
...
#    params.add('rho2', value=np.radians(20.0), min=rho_min, max=rho_max)
    ...
    # These parameters will be fixed at each grid point value:
    ...
    # We will be fitting the rho values.  Set these up with guess values, 
    # and min/max boundary values.
    # Need to first set up strings for rho variables, depending on how many rhos 
    # there are to be fit.
    rho_str = []
    for i_rho in np.arange(n_rho):
        rho_str.append('rho_'+str(i_rho))
        params.add(rho_str[i_rho], value=rho_guess[i_rho], 
                min=rho_min, max=rho_max)
    # rho values will vary in each fit: 
        params[rho_str[i_rho]].vary = True
...
#    rho2_val = []
    ...
    rho_val = np.empty((n_rho, n_iter), dtype=float)
    # Reduced chi2 will be the same for all rho values, as this is a 
    # simultaneous fit.  Will use as weight for rho histograms:
    rho_chi2 = np.empty(n_iter, dtype=float)


# Now run grid.  Opting not to run a list comprehension-type thing, for clarity.

# Header for standard output:
    sys.stdout.write('\n {0:<15s}  {1:>7s}  {2:>8s}  {3:>10s}  {4:>14s}  {5:>14s}\n'.format("Iter", "alpha", "delta", "T1", "Chi2", "Red. Chi2"))
...
# T1 will run from mjd_mid-prec_period/2. to mjd_mid+prec_period/2.
                for i_T1 in np.arange(n_T1):
                    ...
                    i_iter = i_alpha*n_delta*n_T1 + i_delta*n_T1 + i_T1
                    ...
                    for i_rho in np.arange(n_rho):
                        params[rho_str[i_rho]].value = rho_guess[i_rho]
...
#                    params['rho2'].value = np.radians(20.0)
                    ...
                    # Append the fit value of rho, and its associated 
                    # reduced chisq, to existing arrays
                    for i_rho in np.arange(n_rho):
                        rho_val[i_rho][i_iter] = params[rho_str[i_rho]].value
...
# The reduced chi2 value is the same for both rho values:
                    rho_chi2[i_iter] = fit.chisqr
#                    rho_chi2 = np.append(rho_chi2, fit.chisqr)

#####
                    if(np.fmod(i_iter, 16)==0):
                        display_status(i_iter, n_iter, params, fit)
